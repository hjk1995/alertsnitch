...
// Vertica A database that does nothing
type Vertica struct {
	db *sql.DB
}

// ConnectMySQL connect to a MySQL database using the provided data source name
func connectVertica(args ConnectionArgs) (*Vertica, error) {
	if args.DSN == "" {
	...
	connection, err := sql.Open("vertica", "vertica://"+args.DSN)
	if err != nil {
...
func (d Vertica) Save(data *internal.AlertGroup) error {
...
func (d Vertica) unitOfWork(f func(*sql.Tx) error) error {
	tx, err := d.db.Begin()
	if err != nil {
...
func (d Vertica) CheckModel() error {
...
func (Vertica) String() string {
	return "vertica database driver"
}
